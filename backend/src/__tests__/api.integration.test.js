...
describe('API Integration Tests', () => {
  ...
  describe('POST /api/tokens/distribute', () => {
    ...
    it('should return 400 when distributions array is missing', async () => {
      const response = await request(app).post('/api/tokens/distribute').send({}).expect(400);
  ...
  describe('GET /api/events/recent', () => {
    it('should return recent events list', async () => {
      const response = await request(app).get('/api/events/recent').expect(200);
  ...
  describe('POST /api/admin/config', () => {
    it('should return 401 without auth token', async () => {
      await request(app).post('/api/admin/config').send({ key: 'value' }).expect(401);
    });
  ...
  describe('POST /api/sui/token-address', () => {
    it('should return 400 when required fields are missing', async () => {
      const response = await request(app).post('/api/sui/token-address').send({}).expect(400);
  ...
  describe('POST /api/auth/logout', () => {
    it('should return 422 when refreshToken is missing', async () => {
      const response = await request(app).post('/api/auth/logout').send({}).expect(422);
