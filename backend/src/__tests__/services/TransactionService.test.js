...
describe('TransactionService', () => {
  ...
  describe('Job Enqueueing', () => {
    ...
    it('should throw error when type is missing', () => {
      expect(() => service.enqueue({ payload: {} })).toThrow('Transaction type is required');
    });
  ...
  describe('Job Status Transitions', () => {
    ...
    it('should retry job later', () => {
      ...
      expect(updatedJob.nextRunAt).toBeGreaterThanOrEqual(beforeRetry + service.retryDelayMs);
      expect(updatedJob.nextRunAt).toBeLessThanOrEqual(afterRetry + service.retryDelayMs + 100);
    });
  ...
  describe('Job Pruning', () => {
    ...
    it('should prune oldest jobs when exceeding max records', () => {
      ...
      const jobs = [];
      for (let i = 0; i < 15; i++) {
        jobs.push(service.enqueue({ type: 'mint', payload: { amount: `${i * 100}` } }));
      }
