...
router.post("/burn", (req, res) => {
  ...
  if (!coinId) {
    return res
      ...
      .json(
        errorResponse(
          "Please provide a coin ID to burn.",
          { field: "coinId", required: true }
        )
      );
  }
...
router.post("/distribute", (req, res) => {
  ...
  if (!Array.isArray(distributions) || distributions.length === 0) {
    return res
      ...
      .json(
        errorResponse(
          "Please provide at least one recipient in the distributions array.",
          { field: "distributions", format: "Array of {to, amount} objects" }
        )
      );
  }
