  ...
  switch (type) {
    ...
    case 'burn':
      return `Burn tokens from ${
        payload.coinId ? String(payload.coinId).slice(0, COIN_ID_DISPLAY_LENGTH) + '...' : 'coin'
      }`;
    case 'distribute': {
...
export const getSuccessMessage = (action: string): string => {
  const messages: Record<string, string> = {
    ...
    distribute:
      '📤 Distribution request submitted successfully! Recipients will receive tokens soon.',
  };
